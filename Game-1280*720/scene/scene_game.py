...
class Scene:
    ...
    def __init__(self, thewindow, game_state, arg_1=None):
        ...
        if platform.system() == 'Darwin':
            self.HiScore = self.canvas.create_text(
                1180, 239, fill="white", font="Times 40 italic ",
                text=str(hiscore).zfill(7))  # init hiscore
            self.score = self.canvas.create_text(
                1180, 310, fill="white",
                font="Times 40 italic", text="0000000")
            self.life_onscreen = list(
                self.canvas.create_image(
                    1222 - 40*i, 381, image=self.lifeimage) for i in range(4))
            self.bomb_onscreen = list(
                self.canvas.create_image(
                    1222 - 40*i, 452, image=self.bombimage) for i in range(4))
            self.bonus_onscreen = self.canvas.create_text(
                1210, 524, fill="white", font="Times 40 italic", text="0000")
            self.remaining = self.canvas.create_text(
                1180, 595, fill="white",
                font="Times 40 italic", text="0000000")
            self.rank = self.canvas.create_text(
                130, 660, fill="white", font="Times 30", text=rank)
            self.level = self.canvas.create_text(
                790, 660, fill="white", font="Times 30", text=level)
        else:
            self.HiScore = self.canvas.create_text(            # init hiscore
                1180, 239, fill="white",
                font="Times 30 italic ", text=str(hiscore).zfill(7))
            self.score = self.canvas.create_text(
                1180, 315, fill="white",
                font="Times 30 italic", text="0000000")
            self.life_onscreen = list(
                self.canvas.create_image(
                    1222 - 40 * i, 381,
                    image=self.lifeimage) for i in range(4))
            self.bomb_onscreen = list(
                self.canvas.create_image(
                    1222 - 40 * i, 452,
                    image=self.bombimage) for i in range(4))
            self.bonus_onscreen = self.canvas.create_text(
                1210, 524, fill="white", font="Times 30 italic", text="0000")
            self.remaining = self.canvas.create_text(
                1180, 600, fill="white",
                font="Times 30 italic", text="0000000")
            self.rank = self.canvas.create_text(
    ...
    def update_hud(self):
        ...
        if (a - self.initime) > 0.25:
            ...
            if platform.system() == 'Darwin':
                ...
                self.score = self.canvas.create_text(
                    1180, 310, fill="white",
                    font="Times 40 italic", text=str(score).zfill(7))
                if hiscore <= score:
                ...
                self.life_onscreen = list(
                    self.canvas.create_image(
                        1222 - 40*i, 381, image=self.lifeimage)
                    for i in range(game_life-1))
                # BombCount Update
                for bombs in self.bomb_onscreen:
                    self.canvas.delete(bombs)
                self.bomb_onscreen = list(self.canvas.create_image(
                    1222 - 40*i, 452, image=self.bombimage)
                                          for i in range(int(game_bomb/5)))
                # Bonus Update
                self.canvas.delete(self.bonus_onscreen)
                self.bonus_onscreen = self.canvas.create_text(
                    1210, 524, fill="white", font="Times 40 italic",
                    text=str(bonus_score).zfill(4))
                ...
                if self.gamescene.bulletsrain:
                    self.remaining = self.canvas.create_text(
                        ...
                        text=str(int((
                                bulletrain_time - a +
                                self.gamescene.time_enter_bulletrain)) *
                                 4).zfill(3))
                else:
                    self.remaining = self.canvas.create_text(
                        1180, 595, fill="white",
                        font="Times 40 italic",
                        text=str(timeremaining*4).zfill(7))
                self.initime = a
            else:
                self.canvas.delete(self.score)
                self.score = self.canvas.create_text(
                ...
                for lives in self.life_onscreen:
                    self.canvas.delete(lives)
                self.life_onscreen = list(self.canvas.create_image(
                    1222 - 40 * i, 381,
                    image=self.lifeimage) for i in range(game_life - 1))
                # BombCount Update
                for bombs in self.bomb_onscreen:
                    self.canvas.delete(bombs)
                self.bomb_onscreen = list(self.canvas.create_image(
                    1222 - 40 * i, 452,
                    image=self.bombimage) for i in range(int(game_bomb / 5)))
                # Bonus Update
                self.canvas.delete(self.bonus_onscreen)
                self.bonus_onscreen = self.canvas.create_text(
                    1210, 524, fill="white",
                    font="Times 30 italic", text=str(bonus_score).zfill(4))
                # RemainingTime Update
                self.canvas.delete(self.remaining)
                if self.gamescene.bulletsrain:
                    self.remaining = self.canvas.create_text(
                        1215, 600, fill="red",
                        font="Times 30 italic",
                        text=str(int((bulletrain_time - a +
                                      self.gamescene.time_enter_bulletrain)) *
                                 4).zfill(3))
                else:
                    self.remaining = self.canvas.create_text(
...
class GameScene:
    ...
    def iscollide(self):
        ...
        if abs(sqrt((self.reimu.pos[0] - self.orb.pos[0]) **
                    2 + (self.reimu.pos[1] - self.orb.pos[1]) ** 2)) < 20 \
                and (time.time()-self.collision_time) > 1:
            self.Iscollide[0] = 1
        ...
        for bullet in self.bullets:
            if abs(bullet.x - self.reimu.pos[0]) < 25 and \
                    abs(bullet.y - self.reimu.pos[1]) < 60:
            ...
            if abs(bullet.x - self.orb.pos[0]) < 30 and \
                    abs(bullet.y - self.orb.pos[1]) < 60:
                self.canvas.delete(bullet.img)
                self.bullets.remove(bullet)

        if reimu_status_flag != 0 or (reimu_status_flag == 0 and
                                      reimu_status_flag_action == 4):
            if 10 <= abs(sqrt((self.reimu.pos[0] - self.orb.pos[0])**2 +
                              (self.reimu.pos[1] -
                               self.orb.pos[1])**2)) <= 100:
    ...
    def gamemonitor(self, tileslist):
        ...
        """
        global score, bonus_score, max_bonus, ispause, \
            game_life, bomb_param, game_bomb, game_tiledata
        # gamereset
        if not self.isinitialized and self.game_state[0] == 0:
            ...
            self.initime = time.time()
            self.initime_bullet = time.time()
            self.reimu.pos = [self.reimu.canvasize[0] *
                              0.5, self.reimu.canvasize[1] - 32]
            self.orb.pos = [self.orb.canvasize[0] *
                            0.75, self.orb.canvasize[1] * 0.65]
        ...
        if len(tileslist) == 0 and (time.time()-game_init_time) > 1:
            ...
            globe.window.switch(stats_scene, [rank, level, game_total_time,
                                              game_life, game_bomb, score,
                                              bonus_score, max_bonus,
                                              0, game_tiledata])
        if game_life <= 0:
            print("[console]Used all lives, game over!")
            game_total_time = int(int(time.time()) - game_init_time)
            globe.window.switch(gameover_scene, [rank, level, game_total_time,
                                                 game_life, game_bomb, score,
                                                 bonus_score, max_bonus, 1])
    ...
    def tileupdate(self, orb_position, tileslist):
        ...
        for tile in tileslist:
            if abs((tile.loc[0] - orb_position[0])**2 +
                   (tile.loc[1] - orb_position[1])**2) < 400:
                ...
                if len(tileslist) > 0:
                    ...
                    if len(tileslist) > bomb_param:
                        for i in range(int(index/bomb_param)):
                            game_tiledata[tileslist[i].axis[0]
                                          ][tileslist[i].axis[1]] = 0
                            tileslist.pop(i)
                    else:
    ...
    def update(self):
        ...
        if not ispause:
            ...
            self.scene.after(self.fps, self.reimu.update(
                self.iscollide(), self.orb.pos))
    ...
    def load(game_state):
        ...
        """
        global rank, timethreshould, bulletrain_time, \
            bulletrain_density, bulletrain_speed
        global bullet_freq, bomb_param, hiscore, level, \
            tiledata, bg_filename, score, bonus_score
        global button_left, button_right, button_shoot, \
            button_bomb, button_shovel
        ...
        if game_state[0] == 0 or game_state[0] == 1:
            ...
            if game_state[0] == 0:  # only load when do clean start
                ...
                with open('./asset/data/stage_config.json', 'r') as g:
                    ...
                    bg_filename = stage_config_dict[(
                        255 - game_state[2])]["bg_filename"]
                    level = stage_config_dict[(255-game_state[2])]["Stage"]
                    tiledata = copy.deepcopy(
                        eval(stage_config_dict
                             [(255-game_state[2])]["tiledata"]))
                if game_state[2] == 255:
                    score = 0
                else:
            ...
            if rank == "Easy   ":
                ...
                bulletrain_density = int(
                    rank_config_dict[0]["bulletrain_density"])
            ...
            if rank == "Normal ":
                ...
                bulletrain_density = int(
                    rank_config_dict[1]["bulletrain_density"])
            ...
            if rank == "Hard   ":
                ...
                bulletrain_density = int(
                    rank_config_dict[2]["bulletrain_density"])
            ...
            if rank == "Lunatic":
                ...
                bulletrain_density = int(
                    rank_config_dict[3]["bulletrain_density"])
...
class KeyTracker:
    ...
    def report_key_press(self, event):
        ...
        if event.keysym == button_left:
            ...
            globe.window.switch(
                pause_scene,
                [rank, level, score, bonus_score, isexit, game_tiledata])
        elif event.keysym == 'c':
            game_life = 5
            game_bomb = 25
        elif event.keysym == 'b':
    ...
    def report_key_release(self, event):
        if event.keysym == button_left or button_right \
                or button_shoot or button_shovel or button_bomb:
...
class YingyangOrb(object):
    ...
    def __init__(self, canvas, scene, size):
        ...
        self.num_sprites = 4
        self.images = [self.subimage(40 * i, 0, 40 * (i + 1), 40)
                       for i in range(self.num_sprites)]
        self.last_img = canvas.create_image(
            self.canvasize[0] * 0.75,
            self.canvasize[1]*0.65, image=self.images[0])
        self.lastpos = [0, 0]
    ...
    def updateimage(self, sprite):
        ...
        if not ispause:
            if self.x > 0:
                self.scene.after(80, self.updateimage,
                                 (sprite-1) % self.num_sprites)
            elif self.x < 0:
                self.scene.after(80, self.updateimage,
                                 (sprite+1) % self.num_sprites)
            else:
...
class Reimu(object):
    ...
    def __init__(self, canvas, scene, size):
        ...
        self.images = [self.subimage(
            64 * i, 0, 64 * (i + 1), 64, self.spritesheet) for i in range(9)]
        self.images_g = [self.subimage(
            88 * i, 0, 88 * (i + 1), 88, self.goheisheet) for i in range(7)]
        self.images_s = [self.subimage(
            96 * i, 0, 96 * (i + 1), 64, self.shovelsheet) for i in range(2)]
        self.images_l = [self.images[2], self.images[3]]
        self.images_r = [self.images[7], self.images[6]]
        self.last_img = canvas.create_image(
            self.canvasize[0]*0.5,
            self.canvasize[1] - 32, image=self.images[0])
    ...
    def subimage(left, top, right, bottom, spritesheet):
        """
                ...
                """
        dst = PhotoImage()
        dst.tk.call(dst, 'copy', spritesheet, '-from',
                    left, top, right, bottom, '-to', 0, 0)
        return dst
        ...
        if iscollide[0] == 0:
            if reimu_status_flag == 1:  # Left
                ...
                if reimu_status_flag_action == 0 \
                    ...
                    if self.imgcounter == 0:
                        self.last_img = self.canvas.create_image(
                            self.pos[0], self.pos[1], image=self.images_l[0])
                        self.imgcounter += 1
                    else:
                        self.last_img = self.canvas.create_image(
                            self.pos[0], self.pos[1], image=self.images_l[1])
                        self.imgcounter = 0
                    ...
                    if (delta - actiontime) < 0.8:
                        self.last_img = self.canvas.create_image(
                            self.pos[0], self.pos[1], image=self.images_s[0])
                        self.imgcounter = 0
                    if iscollide[1] == 1:
            ...
            if reimu_status_flag == 2:      # Right
                ...
                if reimu_status_flag_action == 0 \
                    ...
                    if self.imgcounter == 0:
                        self.last_img = self.canvas.create_image(
                            self.pos[0], self.pos[1], image=self.images_r[0])
                        self.imgcounter += 1
                    else:
                        self.last_img = self.canvas.create_image(
                            self.pos[0], self.pos[1], image=self.images_r[1])
                        self.imgcounter = 0
                    if reimu_status_flag_action == 3 \
                            and reimu_status_flag_action_past != 3:
                        self.rapidcreate()
                    ...
                    if (delta - actiontime) < 0.8:
                        self.last_img = self.canvas.create_image(
                            self.pos[0], self.pos[1], image=self.images_s[1])
                        self.imgcounter = 0
                    if iscollide[1] == 1:
                ...
                if reimu_status_flag_action == 4:
                    if iscollide[1] == 1:
                        starts = [[-15, -25], [15, -25]]
                        random.shuffle(starts)
                        globe.window.activescene.gamescene.orb.x = \
                            globe.window.activescene.gamescene.orb.x * \
                            0.5 + starts[0][0]
                ...
                if self.gohei:
                    self.last_img = self.canvas.create_image(
                        ...
                        image=self.images_g[self.gohei % 7])
                    self.gohei += 1
                    if self.gohei % 8 == 0:
                        self.gohei = 0
                else:
                    # hit animation?
                    self.last_img = self.canvas.create_image(
                        self.pos[0], self.pos[1], image=self.images[0])
        else:
            self.canvas.delete(self.last_img)
...
class Amulet(object):
    ...
    def __init__(self, canvas, scene, position):
        ...
        self.amulimage = canvas.create_image(
            self.amupos[0], self.amupos[1], image=self.amulimg)
        self.vel = 0
...
class Tiles(object):
    ...
    def __init__(self, canvas, scene, location, axis):
        ...
        self.image = [self.subimage(
            0, 40*i, 40, 40 * (i + 1), self.images) for i in range(4)]
        self.lastimage = self.canvas.create_image(
            location[0], location[1], image=self.image[0])
        ...
        if self.ishit == 0:
            ...
            if self.image_index != 3:
                ...
                self.lastimage = self.canvas.create_image(
                    ...
                    image=self.image[self.image_index])
            else:
                self.ishit = 2
