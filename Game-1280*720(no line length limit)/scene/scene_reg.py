...
class Scene:
        ...
        with open('./asset/data/score.json', 'r') as self.file_r:
            ...
            for i in range(4):
                if platform.system() == 'Darwin':
                    self.canvas.create_text(335, 328 + 82 * i, fill="black", font="Times  50 bold", text=str(score_dict[i]["Name"]))
                    self.canvas.create_text(600, 328 + 82 * i, fill="red", font="Times 50 italic", text=str(score_dict[i]["Score"]))
                    self.canvas.create_text(924, 328 + 82 * i, fill="purple", font="Times 50 italic", text=str(score_dict[i]["Rank"]))
                    self.canvas.create_text(1080, 328 + 82 * i, fill="purple", font="Times 50 bold", text=str(score_dict[i]["Level"]))
                else:
                    self.canvas.create_text(335, 333 + 82 * i, fill="black", font="Times  40 bold", text=str(score_dict[i]["Name"]))
                    self.canvas.create_text(600, 333 + 82 * i, fill="red", font="Times 40 italic", text=str(score_dict[i]["Score"]))
                    self.canvas.create_text(924, 333 + 82 * i, fill="purple", font="Times 40 italic", text=str(score_dict[i]["Rank"]))
                    self.canvas.create_text(1080, 333 + 82 * i, fill="purple", font="Times 40 bold", text=str(score_dict[i]["Level"]))
    ...
    def registration(self):
        ...
        if platform.system() == 'Darwin':
            self.entry = Entry(self.scene, font="Times 50 italic", justify="center", width=32, bg="purple", fg="yellow", disabledbackground="#1E6FBA",
                               disabledforeground="black", highlightbackground="black", highlightcolor="red", highlightthickness=1, bd=0)
            ...
            self.button = tk.Button(text="Submit", command=lambda: self.getvalue(), bg="#cbd020", fg="purple", font="Times 50 bold", bd=0)
            self.canvas.create_window(1035, 660, window=self.button)
        else:
            self.entry = Entry(
                self.scene, font="Times 40 italic", justify="center", width=32, bg="purple", fg="yellow",
                disabledbackground="#1E6FBA", disabledforeground="black", highlightbackground="black", highlightcolor="red", highlightthickness=1, bd=0)
            self.entry.place(width=20, height=500)
            self.canvas.create_window(550, 660, window=self.entry)
            self.entry.insert(0, "Enter your name: (4 chars limit)")
            self.button = tk.Button(text="Submit", command=lambda: self.getvalue(), bg="#cbd020", fg="purple", font="Times 36 bold", bd=0)
            self.canvas.create_window(1035, 660, window=self.button)
